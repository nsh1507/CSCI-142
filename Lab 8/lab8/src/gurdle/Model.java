...
public class Model {
    ...
    public void enterNewGuess(String guess) {
        ...
        if (this.gameState.equals(GameState.ONGOING)){
            if (guess.length() != WORD_SIZE){
                ...
                int row = this.attemptNum;
                for (int loop = 0; loop < guess.length(); loop ++){
                    CharChoice ch = new CharChoice();
                    ch.setChar(guess.charAt(loop));
                    this.guessLetters[row][loop] = ch;
                    this.lettersUsed.add(guess.charAt(loop));
                }
            }
    ...
    public String secret() {
        this.notifyObservers(STATE_MSGS.get(this.gameState));
